...
func (c *ConfigController) ConfigPostHandler(w http.ResponseWriter, r *http.Request) {
	...
	if has("platforms") {
		config.Platforms = req.Platforms
	}
	if has("build_trigger") {
		config.BuildTrigger = req.BuildTrigger
	}
	if has("watched_branch_patterns") {
		config.WatchedBranchPatterns = req.WatchedBranchPatterns
	}
	if has("watched_tag_patterns") {
		config.WatchedTagPatterns = req.WatchedTagPatterns
	}
	if has("dependency_caching") {
		config.DependencyCaching = req.DependencyCaching
	}
	if has("dependency_dirs") {
		config.DependencyDirs = req.DependencyDirs
	}

	// Git Connection
	if has("git_repo") {
		config.GitRepo = req.GitRepo
	}
	if has("git_username") {
		config.GitUsername = req.GitUsername
	}
	if has("git_token") {
		config.GitToken = req.GitToken
	}

	// Webhooks
	if has("webhook_urls") {
		config.WebhookURLs = req.WebhookURLs
	}

	// Scripts
	if has("post_clone_script") {
		config.PostCloneScript = req.PostCloneScript
	}
	if has("pre_test_script") {
		config.PreTestScript = req.PreTestScript
	}
	if has("post_test_script") {
		config.PostTestScript = req.PostTestScript
	}
	if has("pre_build_script") {
		config.PreBuildScript = req.PreBuildScript
	}
	if has("post_build_script") {
		config.PostBuildScript = req.PostBuildScript
	}
	if has("pre_publish_script") {
		config.PrePublishScript = req.PrePublishScript
	}

	// Testing
	if has("test") {
		config.Test = req.Test
	}
	if has("enable_flutter_analyze") {
		config.EnableFlutterAnalyze = req.EnableFlutterAnalyze
	}
	if has("flutter_analyze_args") {
		config.FlutterAnalyzeArgs = req.FlutterAnalyzeArgs
	}
	if has("enable_flutter_test") {
		config.EnableFlutterTest = req.EnableFlutterTest
	}
	if has("flutter_test_args") {
		config.FlutterTestArgs = req.FlutterTestArgs
	}
	if has("enable_flutter_driver") {
		config.EnableFlutterDriver = req.EnableFlutterDriver
	}
	if has("flutter_driver_args") {
		config.FlutterDriverArgs = req.FlutterDriverArgs
	}
	if has("flutter_driver_targets") {
		config.FlutterDriverTargets = req.FlutterDriverTargets
	}

	// Build Settings
	if has("flutter_version") {
		config.FlutterVersion = req.FlutterVersion
	}
	if has("xcode_version") {
		config.XcodeVersion = req.XcodeVersion
	}
	if has("cocoapods_version") {
		config.CocoaPodsVersion = req.CocoaPodsVersion
	}
	if has("project_path") {
		config.ProjectPath = req.ProjectPath
	}
	if has("android_build_format") {
		config.AndroidBuildFormat = req.AndroidBuildFormat
	}
	if has("build_mode") {
		config.BuildMode = req.BuildMode
	}
	if has("android_build_args") {
		config.AndroidBuildArgs = req.AndroidBuildArgs
	}
	if has("ios_build_args") {
		config.IosBuildArgs = req.IosBuildArgs
	}
	if has("web_build_args") {
		config.WebBuildArgs = req.WebBuildArgs
	}
	...
	if has("enable_android_code_signing") {
		config.EnableAndroidCodeSigning = req.EnableAndroidCodeSigning
	}
	if has("enable_google_play_publishing") {
		config.EnableGooglePlayPublishing = req.EnableGooglePlayPublishing
	}
	if has("google_play_track") {
		config.GooglePlayTrack = req.GooglePlayTrack
	}
	if has("update_priority") {
		config.UpdatePriority = req.UpdatePriority
	}
	if has("rollout_fraction") {
		config.RolloutFraction = req.RolloutFraction
	}
	if has("do_not_send_for_review") {
		config.DoNotSendForReview = req.DoNotSendForReview
	}
	if has("submit_as_draft") {
		config.SubmitAsDraft = req.SubmitAsDraft
	}
	if has("publish_even_if_tests_fail") {
		config.PublishEvenIfTestsFail = req.PublishEvenIfTestsFail
	}

	// Linked User-level Assets
	if has("keystore_id") {
		config.KeystoreID = req.KeystoreID
	}
	if has("google_play_credentials_id") {
		config.GooglePlayCredentialsID = req.GooglePlayCredentialsID
	}

	// Notifications
	if has("enable_email_notifications") {
		config.EnableEmailNotifications = req.EnableEmailNotifications
	}
	if has("email_recipients") {
